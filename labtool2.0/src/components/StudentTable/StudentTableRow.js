...
export const StudentTableRow = props => {
  ...
  return (
    <Table.Row key={data.id} className={data.dropped || !data.valid ? 'TableRowForDroppedOutStudent' : 'TableRowForActiveStudent'}>
      {/* Select Check Box */}
      {showColumn('select') && (
      ...
      <Table.Cell key="studentinfo">
        {data.valid ? null : <Popup trigger={<Icon name="warning" color="black" />} content="This student has invalid course registration" />}
        {allowReview ? (
